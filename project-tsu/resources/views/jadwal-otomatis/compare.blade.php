                ...
                <div class="lg:col-span-2 space-y-4">
                    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                        <div class="px-6 py-4 border-b border-slate-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                            ...
                            <button type="button" id="btn-bulk-delete" onclick="submitBulkDelete()" class="hidden px-3 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 text-xs font-bold rounded-lg transition shadow-sm items-center gap-1.5">
                            ...
                            @forelse($trials as $trial)
                                <div class="p-5 hover:bg-slate-50 transition flex items-start gap-4 trial-row" 
                                    ...
                                    <div class="pt-1 flex-shrink-0">
                                        <input type="checkbox" name="trial_ids[]" onchange="handleCheckboxChange()" value="{{ $trial->id }}" 
                        ...
                        <form id="form-bulk-delete" action="{{ route('jadwal.otomatis.delete_multiple_trials') }}" method="POST" class="hidden">
                            @csrf
                            <input type="hidden" name="tahun_akademik_id" value="{{ $selectedTahun ? $selectedTahun->id_tahunakademik : '' }}">
                        </form>
                    </div>
                </div>
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden sticky top-[92px]">
                        ...
                        <div id="comparison-active" class="hidden p-5 space-y-4">
                            <div class="bg-slate-50 border border-slate-200/60 rounded-xl p-4 space-y-4">
                                ...
                                <div id="tradeoff-analysis" class="mt-4 bg-teal-50 border border-teal-100 rounded-lg p-3 text-sm text-teal-800 leading-relaxed shadow-sm">
                                    <div class="font-bold flex items-center gap-1.5 mb-1"><span class="text-lg">🤖</span> Analisis Trade-off AI:</div>
                                    <p id="tradeoff-text" class="text-xs text-teal-700"></p>
                                </div>
...
function handleCheckboxChange() {
    ...
    // Update Bulk Delete Button regardless of how many are checked for comparison
    ...
    if (checkedTrials.length > 0) {
        ...
        bulkCount.innerText = checkedTrials.length;
    } else {
    ...
    generateTradeOffAnalysis(checkedTrials, maxFitness, minConflicts, minSoft);
}

function submitBulkDelete() {
    if (checkedTrials.length === 0) return;
    if (confirm(`Apakah Anda yakin ingin menghapus ${checkedTrials.length} uji coba terpilih?`)) {
        const form = document.getElementById('form-bulk-delete');
        // Clear any previous dynamic inputs
        form.querySelectorAll('.dyn-input').forEach(e => e.remove());
        
        checkedTrials.forEach(t => {
            const input = document.createElement('input');
            input.type = 'hidden';
            input.name = 'trial_ids[]';
            input.value = t.id;
...
function generateTradeOffAnalysis(trials, maxFit, minConf, minSoft) {
    const textEl = document.getElementById('tradeoff-text');
    if (trials.length < 2) return;

    let bestFitTrial = trials.find(t => t.fitness === maxFit);
    let bestConfTrial = trials.find(t => (t.dosenConflicts + t.ruanganConflicts) === minConf);
    let bestSoftTrial = trials.find(t => t.softViolations === minSoft);

    // If one dominates completely
    if (bestFitTrial.id === bestConfTrial.id && bestFitTrial.id === bestSoftTrial.id) {
        textEl.innerHTML = `<strong>${bestFitTrial.label}</strong> mendominasi di semua aspek. Jadwal ini sangat direkomendasikan karena memiliki tingkat kebugaran (fitness) tertinggi dan pelanggaran paling minim.`;
        return;
    }

    let analysis = `Setiap jadwal memiliki keunggulan masing-masing. `;
    if (bestFitTrial.id !== bestConfTrial.id) {
        analysis += `<strong>${bestFitTrial.label}</strong> memiliki skor fitness paling tinggi (${bestFitTrial.fitness}%), yang berarti sebaran jamnya lebih optimal. Namun, <strong>${bestConfTrial.label}</strong> lebih aman secara fundamental karena memiliki bentrok ruang/dosen paling sedikit (${bestConfTrial.dosenConflicts + bestConfTrial.ruanganConflicts} bentrok fatal). `;
    } else {
        analysis += `<strong>${bestFitTrial.label}</strong> adalah kandidat terkuat dengan fitness tertinggi dan bentrok fatal paling sedikit. `;
    }

    if (bestSoftTrial.id !== bestFitTrial.id && bestSoftTrial.id !== bestConfTrial.id) {
        analysis += `Jika kenyamanan dan preferensi dosen (soft constraint) sangat penting bagi Anda, pertimbangkan juga <strong>${bestSoftTrial.label}</strong> yang meminimalkan pelanggaran soft constraint (${bestSoftTrial.softViolations} pelanggaran).`;
    }
