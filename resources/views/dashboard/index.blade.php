...
<body class="bg-gray-50 overflow-x-hidden min-h-screen font-sans transition-colors duration-300">
                        ...
                        <div class="space-y-3">
                            @if($kalenderAkademik->isEmpty())
                                <div class="p-4 text-center text-gray-500 bg-gray-50 rounded-lg border border-gray-100">
                                    ...
                                    <a href="{{ route('settings.academic_calendar.index') }}" class="text-teal-600 hover:text-teal-700 text-xs font-medium mt-1 inline-block">
                                        + Tambah Jadwal
                                    </a>
                                </div>
                            @else
                                @foreach($kalenderAkademik as $event)
                                    <div class="p-3 rounded-lg flex items-start bg-[#87B7C7] text-white">
                                        <svg class="w-5 h-5 mr-3 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                        <div>
                                            <h4 class="font-bold text-sm">{{ $event['title'] }}</h4>
                                            <p class="text-xs opacity-90">{{ $event['date'] }}</p>
                                        </div>
                                    </div>
                                @endforeach
                            @endif
    ...
    <div id="login-success-popup" class="fixed inset-0 z-[100] flex items-center justify-center">
        ...
        <div class="relative bg-white/90 backdrop-blur-sm rounded-3xl p-8 shadow-2xl transform transition-all scale-100 max-w-sm w-full text-center border border-white/50 animate-bounce-in">
            ...
            <button onclick="document.getElementById('login-success-popup').remove()" class="w-full bg-gradient-to-r from-[#0E6973] to-[#095058] hover:from-[#0b565e] hover:to-[#073d42] text-white py-3.5 rounded-xl font-bold text-sm shadow-lg hover:shadow-xl transition-all transform hover:-translate-y-0.5 focus:outline-none ring-4 ring-teal-500/20">
